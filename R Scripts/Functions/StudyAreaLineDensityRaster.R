StudyAreaLineDensityRaster <- function(inputStudyArea,inputLines,cellSize,buffWidth){
  ...
  if(st_crs(inputStudyArea)$proj4string==st_crs(inputLines)$proj4string){
  

  print("Generating grid...")
  #Create template raster from Study Area extent
  ...
  #extract centroids from raster
  rast_point <- rasterToPoints(raster_template, spatial = TRUE) %>% 
    st_as_sf() 
  #create buffers from raster cell centroids
  rast_point_buff <- st_buffer(rast_point,dist=buffWidth)
  
  print("Finding intersections between grid cell centroid buffers and input lines...")
  #find intersection points between buffer boundaries and inputlines
  rast_point_buff_ls <- st_cast(rast_point_buff,"LINESTRING")
  inputLines <- st_geometry(inputLines) %>% st_transform(crs = st_crs(rast_point_buff_ls)$proj4string)#reduce size of dataset and ensure proj4strings are the same
  
  int_points <- st_intersection(rast_point_buff_ls,inputLines) %>% #find intersection of road lines and centroid buffers
    st_cast("MULTIPOINT") %>%
    st_cast("POINT")
  
  print("Splitting input lines by intesections...")
  ...
  grid <- st_make_grid(rast_point_buff,what = "polygons",n = ncells)#split bounding box iof polygon into grid
  grid <- grid[st_intersects(grid,int_points_buff,sparse = F)[,1]]#remove grid cells that do not hintersect with the roads
  printPercent <- seq(1:length(grid),by=(length(grid)/100))
