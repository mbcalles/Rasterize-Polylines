...
#beep =TRUE if you want to have a noise notify you the function is done running (may take a long time)

source("set-up.R")

points <- st_read("input",
                  layer = "VancouverFishnetCentroids") %>% 
  select(ORIG_FID) #read in points of fishnet
...
split_rds <- st_geometry(rds) %>% 
  st_combine() #combine roads into one gemetry
...
#st_write(obj = split_rds, dsn = "output/split_rds_mls.shp")#write object to shapefile
...
#st_write(obj = split_rds_lengths, dsn = "output/split_rds_lengths.shp")#write object to shapefile
